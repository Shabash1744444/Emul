...
// Добавить в начало файла или перед использованием (Скрин 5)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    externalLinks.forEach(link => {
        link.addEventListener('click', async (e) => {
            ...
            // Проверяем наличие плагина и его методов (Скрин 2)
            if (window.NativeBrowser && typeof window.NativeBrowser.open === 'function') {
                try {
                    ...
                    // Добавляем слушатель только если он еще не добавлен
            ...
            // Fallback для браузера или при ошибке плагина
...
async function runDownloadRadar() {
    // Добавляем проверку и логирование (Скрин 1)
    if (!window.NativeFilesystem) {
        console.log('Радар: NativeFilesystem не доступен (тестовый режим)');
        // Для теста в браузере можно раскомментировать:
        // alert('Радар работает только в скомпилированном APK');
        return;
    }
    ...
    try {
        ...
        // Добавляем обработку ошибки чтения
        let result;
        try {
        ...
        // Проверяем что files существует и это массив
        if (!result || !Array.isArray(result.files)) {
            console.log('Радар: Нет доступа к файлам или папка пуста');
            return;
        }
...
function promptRadarInstall(fileObj, Filesystem, Directory) {
    ...
    document.getElementById('radar-install-btn').onclick = async () => {
        ...
        try {
            ...
            // ИСПРАВЛЕНО: Правильная конвертация base64 в File объект (Скрин 3)
            let blob;
            if (fileData.data) {
                // Capacitor возвращает base64 строку
                ...
                // На случай если вернется blob
                blob = fileData.blob;
            } else {
                throw new Error('Нет данных файла');
            }
            
            // Создаем настоящий File объект с именем
        ...
        window.processSingleFileExtended = async function(file) {
            ...
            if ((fileName.endsWith('.rar') || fileName.endsWith('.7z')) && typeof Archive !== 'undefined') {
                ...
                for(let f of fileList) {
                    // Теперь функция readBlobSafe доступна здесь (Скрин 5)
